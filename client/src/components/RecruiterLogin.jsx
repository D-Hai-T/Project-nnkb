...
const RecruiterLogin = () => {
  ...
  const headingText = state === "Login"
    ? t("recruiter.heading.login")
    : isTextDataSubmited
      ? t("recruiter.heading.brand")
      : t("recruiter.heading.signup");
  const subHeadingText = state === "Login"
    ? t("recruiter.subheading.login")
    : isTextDataSubmited
      ? t("recruiter.subheading.brand")
      : t("recruiter.subheading.signup");
  ...
  const handleDrop = (e) => {
    ...
    if (file && file.type.startsWith("image/")) {
      ...
      toast.error(t("recruiter.errors.image"));
    }
  };
  ...
  const onSubmitHandler = async (e) => {
    ...
    if (state === "Sign Up" && !isTextDataSubmited) {
      if (passwordStrength < 3) {
        toast.warning(t("recruiter.errors.passwordWeak"));
        return;
      }
    ...
    try {
      if (state === "Login") {
        ...
        if (data.success) {
          ...
          toast.success(t("recruiter.success.login"));
          setTimeout(() => {
            setShowRecruiterLogin(false);
            navigate("/dashboard");
          }, 1000);
        } else {
          toast.error(data.message || t("recruiter.errors.loginFailed"));
        }
      } else {
        ...
        if (data.success) {
          ...
          setCompanyToken(data.token);
          localStorage.setItem("companyToken", data.token);
          toast.success(t("recruiter.success.signup"));
          setTimeout(() => {
            setShowRecruiterLogin(false);
            navigate("/dashboard");
          }, 1000);
        } else {
          toast.error(data.message || t("recruiter.errors.signupFailed"));
        }
      }
    } catch (error) {
      toast.error(error.message || t("recruiter.errors.generic"));
    } finally {
      setIsLoading(false);
    }
  };
      ...
      >
        <div className="relative overflow-hidden bg-white rounded-3xl shadow-2xl">
          ...
          <div className="px-8 pt-16 pb-4">
            ...
            <p className="text-sm text-center text-gray-500">{subHeadingText}</p>
          </div>
            ...
            >
                    ...
                    {image ? (
                      <div className="relative w-full h-full">
                        ...
                        >
                          ...
                          <span className="text-xs font-medium text-white mt-1">{t("recruiter.logo.remove")}</span>
                        </div>
                      </div>
                    ) : (
                      <label htmlFor="logo-upload" className="flex flex-col items-center justify-center w-full h-full cursor-pointer">
                        ...
                        <span className="text-sm font-medium text-blue-600">{t("recruiter.logo.upload")}</span>
                        <span className="text-xs text-gray-500 mt-1">{t("recruiter.logo.helper")}</span>
                        <input
                          id="logo-upload"
                          type="file"
                          accept="image/*"
                ...
                <>
                  {state !== "Login" && (
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        type="text"
                        placeholder={t("recruiter.fields.companyPlaceholder")}
                        required
                      />
                    </div>
                  )}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      type="email"
                        placeholder={t("recruiter.fields.emailPlaceholder")}
                      required
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder={
                          state === "Login"
                            ? t("recruiter.fields.passwordPlaceholderLogin")
                            : t("recruiter.fields.passwordPlaceholderSignup")
                        }
                        required
                      />
                      <button 
                    ...
                    {state === "Sign Up" && password && (
                      <div className="mt-2">
                        ...
                        <p className="text-xs mt-1 text-gray-500">
                          {passwordStrength === 0 && t("recruiter.password.states.empty")}
                          {passwordStrength === 1 && t("recruiter.password.states.veryWeak")}
                          {passwordStrength === 2 && t("recruiter.password.states.weak")}
                          {passwordStrength === 3 && t("recruiter.password.states.good")}
                          {passwordStrength === 4 && t("recruiter.password.states.strong")}
                          {passwordStrength === 5 && t("recruiter.password.states.veryStrong")}
                        </p>
                      </div>
                    )}
                  </div>
              ...
              {state === "Login" && (
                <div className="flex justify-end">
                  ...
                  >
                    {t("recruiter.actions.forgotPassword")}
                  </button>
                </div>
              )}
              ...
              {isLoading ? (
                <span className="flex items-center justify-center">
                  <svg className="w-5 h-5 mr-2 animate-spin" viewBox="0 0 24 24">
                    ...
                    {t("recruiter.actions.processing")}
                </span>
              ) : state === "Login" ? (
                t("recruiter.actions.signIn")
              ) : isTextDataSubmited ? (
                t("recruiter.actions.createAccount")
              ) : (
                t("recruiter.actions.continue")
              )}
            </button>
                ...
                <div className="mt-5">
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-sm">
                      <span className="px-3 text-gray-500 bg-white">{t("recruiter.actions.orContinue")}</span>
                    </div>
                  </div>
          ...
          <div className="py-5 bg-gray-50 border-t border-gray-100 rounded-b-3xl">
            <div className="flex justify-center px-8">
              <p className="text-sm text-gray-600">
                {state === "Login"
                  ? t("recruiter.actions.noAccount")
                  : t("recruiter.actions.haveAccount")}
                <button
                  type="button"
                  onClick={() => switchMode(state === "Login" ? "Sign Up" : "Login")}
                  className="font-medium text-blue-600 hover:text-blue-800 transition-colors"
                >
                  {state === "Login" ? t("recruiter.actions.signUpNow") : t("recruiter.actions.signInShort")}
                </button>
              </p>
            </div>
          </div>
