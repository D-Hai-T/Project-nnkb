...
const JobListing = () => {
  ...
  const categorySummary =
    selectedCategory.length > 0
      ? t("jobListing.categoriesCount", { count: selectedCategory.length })
      : "";
  const locationSummary =
    selectedLocation.length > 0
      ? t("jobListing.locationsCount", { count: selectedLocation.length })
      : "";
  const filterSummary = [categorySummary, locationSummary]
    .filter(Boolean)
    .join(` ${t("jobListing.and")} `);
  const formatCategory = (value) => {
    const key = value.toLowerCase().replace(/[^a-z0-9]+/g, "_");
    return t(`jobCategories.${key}`, { defaultValue: value });
  };
            ...
            <>
              ...
              {isSearched && (searchFilter.title !== "" || searchFilter.location !== "") && (
                <div className="mb-8 bg-gray-50 p-4 rounded-lg">
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="font-bold text-lg text-gray-800">{t("jobListing.currentSearch")}</h3>
                    <button 
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="font-bold text-lg text-gray-800">{t("jobListing.categories")}</h4>
                  {selectedCategory.length > 0 && (
                    <button 
              ...
              <div className="mb-4">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="font-bold text-lg text-gray-800">{t("jobListing.locations")}</h4>
                  {selectedLocation.length > 0 && (
                    <button 
                ...
                {JobLocations.length > 5 && (
                  ...
                  >
                    {showAllLocations
                      ? t("jobListing.showLess")
                      : t("jobListing.showAll", { count: JobLocations.length })}
                  </button>
                )}
              </div>
            </>
      ...
      <section className="w-full lg:w-3/4 pl-0 lg:pl-8">
        <div className="mb-8">
          <h3 className="font-bold text-3xl md:text-4xl text-gray-900 mb-2" id="job-list">
            {t("jobListing.latestJobs")}
          </h3>
          <p className="text-gray-600">{t("jobListing.latestJobsSubtitle")}</p>
        </div>
        ...
        <div className="lg:hidden mb-6">
          <div className="relative">
            <input
              type="text"
              placeholder={t("jobListing.mobilePlaceholder")}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          <p className="text-gray-600 mb-2 sm:mb-0">
            {t("jobListing.showingJobs", { count: filterJobs.length })}
            {filterSummary && (
              <span className="text-sm ml-2">
                {t("jobListing.filteredBy", { details: filterSummary })}
              </span>
            )}
          </p>
          <div className="flex items-center">
            <label htmlFor="sort" className="text-gray-600 mr-2 text-sm">
              {t("jobListing.sortLabel")}
            </label>
            <select
              id="sort"
              className="border border-gray-300 rounded-md px-3 py-1 text-sm focus:ring-primary focus:border-primary"
            >
              <option>{t("jobListing.sort.recent")}</option>
              <option>{t("jobListing.sort.salary")}</option>
              <option>{t("jobListing.sort.popular")}</option>
            </select>
          </div>
        </div>
        ...
        <div className="relative min-h-[400px]">
          {filterJobs.length === 0 ? (
            ...
            >
              ...
              <h4 className="text-xl font-medium text-gray-700 mb-2">{t("jobListing.emptyTitle")}</h4>
              <p className="text-gray-500 mb-4">{t("jobListing.emptySubtitle")}</p>
              <button 
                onClick={clearAllFilters}
