...
const ViewApplications = () => {
  ...
  const text = t("viewApplications", { returnObjects: true });
      ...
      case "Accepted":
        return (
          <div className="flex items-center gap-1 bg-emerald-50 text-emerald-600 px-3 py-2 rounded-full text-sm font-medium">
            <Check size={14} />
            <span>{text.status.accepted}</span>
          </div>
        );
      case "Rejected":
        return (
          <div className="flex items-center gap-1 bg-red-50 text-red-600 px-3 py-2 rounded-full text-sm font-medium">
            <X size={14} />
            <span>{text.status.rejected}</span>
          </div>
        );
      default:
        return (
    ...
    <div className="bg-gray-50 min-h-screen py-8">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-gray-500">{text.subheading}</p>
        </motion.div>
        ...
        >
          ...
          {/* Table */}
          <div className="overflow-x-auto overflow-y-visible max-h-[70vh] w-full">
            <table className="w-full">
              <thead className="bg-gray-50 text-gray-700">
                <tr>
                  <th className="py-4 px-5 text-left text-sm font-semibold">{text.table.number}</th>
                  <th className="py-4 px-5 text-left text-sm font-semibold">
                    <div className="flex items-center gap-2">
                      <User size={16} />
                      <span>{text.table.applicant}</span>
                    </div>
                  </th>
                  <th className="py-4 px-5 text-left text-sm font-semibold max-md:hidden">
                    <div className="flex items-center gap-2">
                      <Briefcase size={16} />
                      <span>{text.table.job}</span>
                    </div>
                  </th>
                  <th className="py-4 px-5 text-left text-sm font-semibold max-md:hidden">
                    <div className="flex items-center gap-2">
                      <MapPin size={16} />
                      <span>{text.table.location}</span>
                    </div>
                  </th>
                  <th className="py-4 px-5 text-left text-sm font-semibold">
                    <div className="flex items-center gap-2">
                      <FileText size={16} />
                      <span>{text.table.resume}</span>
                  </div>
                </th>
                  <th className="py-4 px-5 text-left text-sm font-semibold">{text.table.status}</th>
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan="6">
                      <div className="flex flex-col items-center justify-center py-16">
                        ...
                        />
                        <h3 className="text-lg font-medium text-gray-700 mb-1">{text.empty.title}</h3>
                        <p className="text-gray-500 text-center mb-2">{text.empty.description}</p>
                            ...
                            <AnimatePresence>
                              {activeDropdown === index && (
                                <motion.div 
                                  ...
                                  className="absolute right-0 top-full mt-1 z-50 w-36 bg-white border border-gray-100 rounded-lg shadow-lg overflow-visible drop-shadow-xl"
                                >
                                  <button
                                    onClick={() => {
