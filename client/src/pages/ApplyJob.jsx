...
const ApplyJob = () => {
  ...
  const text = t("applyJob", { returnObjects: true });
  ...
  const fetchJob = async () => {
    try {
      ...
      setIsLoading(false);
      toast.error(t("common.notifications.jobFetchFailed"));
    }
  };
  ...
  const applyHandler = async () => {
    try {
      if (!userData) {
        return toast.error(t("common.notifications.loginRequired"));
      }

      if (!userData.resume) {
        return toast.error(t("common.notifications.resumeRequired"));
      }
      ...
      toast.error(t("common.notifications.applyError"));
    }
  };
                ...
                >
                  ...
                  {!isAlreadyApplied && (
                    <p className="mt-2 text-blue-100 text-sm">
                      {userData?.resume ? text.resumeReady : text.resumeReminder}
                    </p>
                  )}
                </motion.div>
              </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="lg:w-2/3">
                ...
                >
                  <h2 className="text-2xl font-bold text-gray-800 mb-6">{text.jobDescription}</h2>
                  <div 
                ...
                {jobData?.requirements && (
                  <motion.div
                    ...
                    className="bg-white rounded-xl shadow-md p-8 mb-8"
                  >
                    <h2 className="text-2xl font-bold text-gray-800 mb-6">{text.requirements}</h2>
                    <div 
                      className="prose max-w-none text-gray-700"
                ...
                {jobData?.benefits && (
                  ...
                  >
                    <h2 className="text-2xl font-bold text-gray-800 mb-6">{text.benefits}</h2>
                    <div 
                      className="prose max-w-none text-gray-700"
              ...
              <div className="lg:w-1/3 space-y-6">
                ...
                >
                  <h3 className="text-xl font-bold text-gray-800 mb-4">
                    {text.companyAbout.replace("{{company}}", jobData?.companyId?.name || "")}
                  </h3>
                  ...
                  >
                    {text.viewProfile} <FiExternalLink className="ml-1" />
                  </a>
                </motion.div>
                ...
                <motion.div
                  ...
                  className="bg-white rounded-xl shadow-md p-6"
                >
                  <h3 className="text-xl font-bold text-gray-800 mb-4">{text.similarJobs}</h3>
                  <div className="space-y-4">
                    {similarJobs.length > 0 ? (
                      ...
                      <p className="text-gray-500">{text.noSimilarJobs}</p>
                    )}
                  </div>
                </motion.div>
                ...
                >
                  <h3 className="text-xl font-bold text-gray-800 mb-4">{text.quickApply}</h3>
                  <button
                    onClick={applyHandler}
                    disabled={isAlreadyApplied}
