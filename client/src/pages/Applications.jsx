...
const Applications = () => {
  ...
  const text = t("applicationsPage", { returnObjects: true });
  ...
  const updateResume = async () => {
    try {
      if (!resume) {
        toast.error(text.resume.selectError);
        return;
      }
  ...
  const statusFilters = [
    ...
    { value: "accepted", label: text.filters.accepted },
    { value: "pending", label: text.filters.pending },
    { value: "rejected", label: text.filters.rejected },
  ];
  const selectedStatusLabel =
    statusFilters.find((item) => item.value === selectedStatus)?.label || "";
    ...
    <div className="min-h-screen bg-slate-900 relative overflow-hidden">
      ...
      >
        ...
        >
          ...
          <p className="text-slate-400 text-lg">{text.heroSubtitle}</p>
        </motion.div>
        ...
        >
          {[
            { label: text.stats.total, value: stats.total, icon: Briefcase, color: "from-slate-600 to-slate-700" },
            { label: text.stats.accepted, value: stats.accepted, icon: CheckCircle, color: "from-emerald-600 to-emerald-700" },
            { label: text.stats.pending, value: stats.pending, icon: Clock, color: "from-amber-600 to-amber-700" }
          ].map((stat, index) => (
            <motion.div
              key={stat.label}
          ...
          <div className="relative bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl overflow-hidden">
            ...
            <div className="p-6">
              <div className="flex items-center mb-6">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-white">{text.resume.title}</h3>
                  <p className="text-slate-400">{text.resume.subtitle}</p>
                </div>
              </div>
                    ...
                    <label className="relative cursor-pointer group">
                      <div className="flex items-center px-4 py-3 bg-slate-700/50 border border-slate-600/50 rounded-lg hover:bg-slate-700/80 hover:border-slate-500/50 transition-all duration-200">
                        ...
                        <span className="text-slate-300 group-hover:text-white transition-colors">
                          {resume ? resume.name : text.resume.select}
                        </span>
                      </div>
                      <input
                        type="file"
            ...
            <div className="p-6">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6">
                <div className="flex items-center mb-4 lg:mb-0">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-white">{text.trackerTitle || text.heroTitle}</h3>
                    <p className="text-slate-400">{text.trackerSubtitle}</p>
                  </div>
                </div>
                ...
                {filteredApplications?.length > 0 ? (
                          ...
                          <div className="relative flex items-center justify-between p-4 bg-slate-700/30 border border-slate-600/30 rounded-lg hover:bg-slate-700/50 hover:border-slate-500/50 transition-all duration-300">
                            ...
                            <div className="flex-shrink-0">
                              <span className={`inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-medium ${statusConfig.bg} ${statusConfig.text} ${statusConfig.border} border backdrop-blur-sm`}>
                                ...
                                {statusConfig.label || text.filters.pending}
                              </span>
                            </div>
                          </div>
                        </motion.div>
                  ...
                  >
                    ...
                    <h3 className="text-xl font-semibold text-white mb-2">
                      {selectedStatus === 'all'
                        ? text.empty.all
                        : t("applicationsPage.empty.filtered", { status: selectedStatusLabel })}
                    </h3>
                    <p className="text-slate-400 mb-8 max-w-md mx-auto">
                      {selectedStatus === 'all'
                        ? text.empty.descriptionAll
                        : t("applicationsPage.empty.descriptionFiltered", { status: selectedStatusLabel })}
                    </p>
