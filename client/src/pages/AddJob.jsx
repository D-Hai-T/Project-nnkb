...
const AddJob = () => {
  ...
  const text = t("addJob", { returnObjects: true });
  ...
  const onSubmitHandler = async (e) => {
    ...
    if (!isFormValid) {
      toast.error(text.validation.required);
      return;
    }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8 mt-8">
        <h2 className="text-2xl md:text-3xl font-bold text-primary">{text.heading}</h2>
        <p className="text-gray-500 mt-1">{text.subheading}</p>
      </div>

      {/* Progress Bar */}
      <div className="mb-10">
        <div className="flex justify-between mb-2">
          <span className="text-sm font-medium text-gray-700">{text.steps.basic}</span>
          <span className="text-sm font-medium text-gray-700">{text.steps.details}</span>
          <span className="text-sm font-medium text-gray-700">{text.steps.preview}</span>
        </div>
        ...
        >
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
            <div className="flex items-center mb-6">
              ...
              <h3 className="text-xl font-semibold text-gray-800">{text.steps.basic}</h3>
            </div>
            
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {text.form.jobTitle} <span className="text-red-500">*</span>
                </label>
                <div className="relative">
                  <input
                    type="text"
                    placeholder={text.form.jobTitlePlaceholder}
                    value={title}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {text.form.salary} <span className="text-red-500">*</span>
                </label>
                <div className="relative">
                  ...
                  <input
                    type="number"
                    min={0}
                    placeholder={text.form.salaryPlaceholder}
                    value={salary || ''}
                ...
                <p className="mt-1 text-xs text-gray-500">{text.form.salaryHelper}</p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
            <div className="flex items-center mb-6">
              ...
              <h3 className="text-xl font-semibold text-gray-800">{text.steps.details}</h3>
            </div>
            
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {text.form.description} <span className="text-red-500">*</span>
                </label>
                <div 
                  ref={editorRef}
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none bg-white transition-all duration-200"
                  >
                    <option value="Beginner level">{text.levelOptions.beginner}</option>
                    <option value="Intermediate level">{text.levelOptions.intermediate}</option>
                    <option value="Senior level">{text.levelOptions.senior}</option>
                  </select>
                </div>
              </div>
            </div>
        ...
        >
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
            <div className="flex items-center mb-6">
              ...
              <h3 className="text-xl font-semibold text-gray-800">{text.steps.preview}</h3>
            </div>
            ...
            <div className="space-y-6">
              <div className="p-6 border border-dashed border-gray-300 rounded-xl bg-gray-50">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900">{title || text.form.jobTitle}</h3>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm text-gray-500">{text.preview.salarySuffix}</p>
                  </div>
                </div>
                
                <div className="border-t border-gray-200 pt-4 mt-4">
                  <h4 className="font-medium text-gray-700 mb-2">{text.preview.descriptionTitle}</h4>
